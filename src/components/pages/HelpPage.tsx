...
const CHANGELOG = [
  {
    ...
    badge: 'emerald',
      changes: [
        '知识库：新增 8 个常用 ESP32/STM32 MCU 实体和 33 个教学元器件实体',
        '检索：正式知识包优先于 legacy 兼容包，未知型号不会静默匹配相似器件',
        '生成：方案与固件提示词只注入当前需求/BOM 命中的硬件事实、接口、地址和限制',
        '证据：实体记录来源、可信状态、包版本和关键事实证据，当前统一标记为 reviewed',
        '兼容：保留原有芯片目标、驱动模板、自定义芯片、PDF 上传和已有项目数据',
        '范围：本版本不启用官方资料联网同步或在线按需检索，未收录器件可继续上传 PDF',
      ],
  },
  {
    version: 'v2.3.1',
  ...
  },
  {
    ...
    changes: [
      ...
      '重构：本机服务拆分配置、HTTP、工作区路径安全和 AI 服务商适配模块',
      '扩展：保留 call / listModels 服务商适配契约，当前不提供公共云端 AI API',
      '测试：补充项目归档和项目状态行为测试，并保留完整本地服务冒烟测试',
    ],
  },
  {
    version: 'v1.5.6',
