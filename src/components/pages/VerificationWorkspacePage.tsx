...
function ActionBar({ state, label, onRun, disabled = false }: { state: CheckState; label: string; onRun: () => void; disabled?: boolean }) {
  ...
  return <div className="flex flex-wrap items-center gap-2"><button type="button" onClick={onRun} disabled={running || disabled} className="inline-flex items-center gap-2 rounded-lg bg-cyan-600 px-3 py-2 text-xs font-medium text-white transition-colors hover:bg-cyan-500 disabled:cursor-not-allowed disabled:opacity-50">{running ? <Loader2 size={13} className="animate-spin" /> : state === 'passed' ? <RefreshCw size={13} /> : <Play size={13} />}{running ? '执行中…' : state === 'passed' ? `重新${label}` : label}</button>{running && <span className="text-xs text-[var(--text-muted)]">请等待当前检查结束</span>}</div>
}
