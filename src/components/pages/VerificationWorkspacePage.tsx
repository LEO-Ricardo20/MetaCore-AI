...
export default function VerificationWorkspacePage() {
  ...
  useEffect(() => {
    // `stale` means the input changed and the old result must be discarded;
    // `invalid` is a completed check with blocking findings and must remain visible.
    const consistencyStale = consistencyArtifactStatus === 'stale'
    const codeStale = codeArtifactStatus === 'stale'
    const buildStale = buildArtifactStatus === 'stale'
    const releaseStale = releaseArtifactStatus === 'stale'
