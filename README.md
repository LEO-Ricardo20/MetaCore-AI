...
当前版本：**v2.3.0**，发布日期为 `2026-08-19`。

2.3.0 完成了从需求、硬件方案、引脚、BOM、接线到固件、流程、验证和导出的阶段化工作流，统一接入 Session、Job、SSE、取消、重试和产物过期状态，并使用仓库内 ESP32 示例验证真实工程扫描及 PlatformIO 固件构建。确定性 Mock Provider 仅用于生成编排 E2E，不代表真实 DeepSeek 调用；本次真实固件构建也不代表已经向物理开发板烧录。
